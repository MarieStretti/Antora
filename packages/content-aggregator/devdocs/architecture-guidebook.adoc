...
* `abspath` - absolute path of source file (only set for local files)
* `basename`
* `stem`
* `extname`
* `mediaType` - resolved from the MIME types catalog by the value of `extname`
* `origin` - an object shared by all files taken from the same branch of the same repository; includes `editUrlPattern`
* `editUrl` - takes the reader to a page where the source can be edited (local if worktree, otherwise hosted)
