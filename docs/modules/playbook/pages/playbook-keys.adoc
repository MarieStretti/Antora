...
By default, it's set to false (i.e., turned "`off`").
When true (i.e., turned "`on`"), it will remove the destination path recusively before generating the site.
This key only applies to the filesystem (fs) providers currently.
...
* xref:output-keys.adoc#output-dir[Specify an output directory]
...
When no destinations are specified, Antora publishes the site to the local filesytem at the location specified by the dir key or, if the dir key is not specified, the default output directory location ([.path]_build/site_).

To disable publishing entirely, including the default output, set destinations to an empty array (i.e., `[]`).
...
* xref:output-keys.adoc#archive-path[Publish site as a ZIP file]
* xref:output-keys.adoc#multiple-destinations[Specify multiple destinations]

==== path

The path key designates the target location where the output file(s) are to be written.
The value gets interpretted appropriately by each provider.
...
* xref:output-keys.adoc#archive-path[Specify ZIP file]
* xref:output-keys.adoc#multiple-destinations[Specify relative and absolute provider paths]
