...
If you've installed Antora globally using `npm`, and you encounter the error `Cannot find module 'isomorphic-git'`, you need to set the `NODE_PATH` environment variable so the custom credential manager can access Antora's dependencies.
