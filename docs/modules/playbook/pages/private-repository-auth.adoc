...
Authentication credentials consist of either a username and password pair or a personal access (i.e., OAuth) token.

If you have two-factor authentication enabled for your account, you typically have to use a token.
...
`<credentials>` consists of either a username and password pair (i.e., `username:password`) or a personal access (i.e., OAuth) token (i.e., `token:`).
...
Specifically, if the protocol (e.g., `https`) and hostname match (and, optionally, the path), then the credentials are returned to credential manager.
...
Instead of looking for the credential store at the default path(s), you can instruct Antora to look for the file in different location using either the `--git-credentials-path` CLI option or `GIT_CREDENTIALS_PATH` environment variable.
...
This strategy is most useful in a CI environment.
...
Antora no longer supports public/private key authentication over SSH using an SSH agent.
Instead, Antora transparently converts SSH URLs to HTTPS URLs.
It then proceeds as if an HTTPS URL was given.
This means users don't have to update their playbook files, even if the playbook uses SSH URLs.
