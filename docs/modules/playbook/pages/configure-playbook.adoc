...
A playbook is usually located at the root of a xref:set-up-existing-playbook.adoc[playbook project] and is written in YAML, a common configuration language for configuring automated tasks.
...
// site root: The name of the component to use as the root of the site.
...
The key accepts valid URIs or local filesystem paths.
Filesystem paths can be relative to the location of the playbook or absolute.

==== branches

The branches key accepts a comma-separated list of branch names.
