...
<8> UI key
<9> <<ui-bundle-key,UI bundle repository key and value>>
<10> Published output key
<11> Output directory key and value
<12> Output destinations key and value(s)
<13> Output provider key and value
<14> Output path key and value
...
These keys tell Antora what protocol provider to use when publishing the site, such as SSH, and where to publish the site.
...
Remove destination path before publishing (fs only).
format: Boolean
default: false
arg: clean
////
...
You can publish a site to multiple destinations.

==== provider

The provider key specifies the protocol Antora should use for writing the generated site.
The default, implicit provider is the filesystem (`fs`).
You can use the `archive` provider to publish the site as a ZIP archive.

==== path

The path key allows you to add file paths per provider.
For example, if using the `archive` provider, you can specify the name of the ZIP file with the path key.

.Provider and path keys (excerpt)
[source,yaml,subs=attributes+]
----
output:
 dir: launch/sites
...
In the example above, the archived site will be located at [.path]_launch/sites/blue.zip_ and the non-archived site will be located at [.path]_launch/sites/red_.

When no path information is specified for the `archive` provider, the site will be saved to [.path]_site.zip_.
