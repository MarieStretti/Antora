...
The default playbook is the file [.path]_site.yml_ at the root of a xref:set-up-existing-playbook.adoc[playbook project].
...
----
site: # <1>
  title: Docs Demo Site # <2>
content: # <3>
- repository: https://github.com/my-antora-demo/server-docs # <4>
  branches: v* # <5>
- repository: https://github.com/my-antora-demo/aspnet-docs
  branches: master
theme: # <6>
  repository: https://github.com/my-antora-demo/docs-site-ui # <7>
----
<1> Site key
<2> Site title key
<3> Content key
<4> Content repository key
<5> Content branches key
<6> Theme key
<7> Theme respository key

The content key is likely the one you'll tune most often.
It contains an array of repository specifications.

// public
At a minimum, you must specify the location of each repository using the `repository` key.
