...
The `url` value must be an absolute URI.
The means the value must start with `https://`, `http://`, `ftp://`, `file://`, etc.
Do not include a trailing slash at the end of the value.
...
== Add a Google analytics account key
