...
* xref:configure-ui.adoc#remote-bundle[Fetch and use a remote UI bundle]
* xref:configure-ui.adoc#local-bundle[Load a local UI bundle]

[#ui-start-path-key]
=== start_path

When the UI files aren't stored at the root of the bundle, specify the bundle relative path to the UI files using `start_path`.
...
The default_layout key applies a layout template to pages that don't specify a layout.

Learn more:

* xref:configure-ui.adoc#default-layout[Specify a default UI layout template in the playbook]

[#ui-output-dir-key]
=== output_dir

The output directory path where the UI files are written.
When no UI `output_dir` is explicitly specified, the files are published to a directory designated by a single underscore ( +_+ ) that is relative to the site root.
