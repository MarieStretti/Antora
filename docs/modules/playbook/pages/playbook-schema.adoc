...
The bundle relative path to the UI files when they aren't stored at the root of the bundle.
...
The default layout key applies a layout template to pages that don't specify a layout.
...
When the UI `output_dir` isn't explicitly specified, the files are published to a directory designated by a single underscore ( +_+ ) that is relative to the site root.
