...
In this case, `asciidoctor-emoji-macro` is the name of an installed node module and is thus available on the require path (either in the node_modules directory for the playbook project or in the global node_modules directory).
...
.An  register function of a scoped Asciidoctor extension
[source,js]
----
...
Here's an example showing how use the `antora` CLI to preload multiple Asciidoctor extensions:

 $ antora -r ./lib/shout-block -r asciidoctor-emoji-macro site.yml

The `-r` option can also be used for other purposes as well, such as to alter global state or override Antora components.
