...
When no branches are specified for a sources `url`, Antora will use the [.term]_default branches set_.
...
Branch names can be separated by commas (`,`) and listed on the same line as `branches`.
...
The tags key (`tags`) accepts the same value patterns as the branches key.

=== Add a global sources tag filter

If you want to the same tag filter applied to all the `url` entries that don't have an individual value defined `branches` or `tags` for the , then assign a value to  `tags` directly under the `content` category.

.Assign a tag filter to all sources
[source,yaml]
----
content:
  tags: v*
...
Antora will find and collect all of the the tags that begin with `v` from the content repositories that don't have branches or tags keys explicitly set on them.

=== Use branches and tags from the same content repository

In this example, we'll fetch the `master` branch from a content repository.
From the same repository, we'll also fetch all of the `v1.x` tags but ignore any tags that start with `v` and end with `-beta`.
