...
** [x] By name
** [x] By glob pattern
* [x] How to mix local and remote branches.
...
Alternatively, each branch name can be specified on its own line, using a hyphen (`-`) list marker, and listed beneath the branches key.
...
=== Mix local and remote branches

You can generate your site using remote branches and a local branch of a component.
In this example, we'll fetch the `v1.*` and `v2.*` branches from Component A's remote content repository.
We'll also load the branch `v3.0.0-beta` from a local repository.

.Use remote and local branches of the same component
[source,yaml]
----
content:
  sources:
...
With `start_path` specified, Antora won't collect any files outside of [.path]_https://github.com/org/repo/packages/docs_.
