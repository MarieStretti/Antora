...
If you want to apply the same tag filter to all source entries that don't declare the `tags` key explicitly, assign a value to the `tags` key directly on the `content` category.
Unlike branches, the default `tags` key does not have a default value (i.e., no tags are used by default).
...
Antora will find and collect all tags that begin with `v` for all content sources that don't define an override for `tags`.
