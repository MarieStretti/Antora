...
To disable this default, either specify at least one destination under the destinations key or set the destinations key to an empty array (i.e., `[]`).
...
When you need to publish to multiple destinations, you'll need to use the destinations key.
...
.Built-in providers
* Filesystem (`fs`) -- Publishes the output files to a directory.
* Archive (`archive`) -- Publishes the output files to a ZIP archive file.
