...
A module's attachment files should be saved in its xref:module.adoc#attachments-dir[attachments folder].
...
Next is the target.
Put the path of the image relative to the xref:module.adoc#images-dir[image catalog] in that slot (no need for an attribute prefix).
...
Next is the target.
Put the path of the video relative to the xref:module.adoc#videos-dir[video catalog] in that slot (no need for an attribute prefix).
