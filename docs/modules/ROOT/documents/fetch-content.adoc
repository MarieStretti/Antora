...
. The list of repositories is read from a playbook ([.path]_site.yml_).

. Using git (via NodeGit), these repositories are cloned or updated and stored under [.path]_build/sources_
...
. Each file in a branch is associated with the component name and version defined in the [.path]_component.yml_ at the root of the branch.
...
NOTE: [.path]_build/sources_ may change to [.path]_build/content_ in the future.
