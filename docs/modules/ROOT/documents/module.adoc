...
A [.term]_module_ is a discrete bundle of content, including text, images, and other source materials, organized in a hierarchy of folders by content type, then by optional topic, tag, or category if needed.
...
As more content is created and your documentation team grows, more often than not, you want to start grouping files into folders so that they're easier to find and manage.
...
It's easy to detach a module from a component and set it up as its own documentation component.
...
In addition to the ROOT module, you can create as many other modules in a component as you need.
All of a component's modules must be stored in [.path]_modules/_.

The following diagram illustrates a documentation component with few example folders and files added for context.
...
We'll start with the ROOT module, since it is required.
...
Simple documentation components may never need more than a ROOT module, however, if your project grows, you can easily add more modules and gradually redistribute the content without having to restructure the project.
...
The ROOT module, as well as any other modules you create, should include the appropriate subdirectory when a module contains the corresponding content type.

For instance:

* AsciiDoc documents go in the [.path]_pages/_ folder.
* Images go in the [.path]_assets/images/_ folder.
* Examples (often code snippets) go in the [.path]_examples/_ folder.
...
Any attachment, image, and video files associated with a module must be stored in that module's [.path]_assets/_ directory and the corresponding content or media type subdirectory.
...
This is managed automatically by Antora and, when viewing the file in an editor, the adjacent [.path]_attributes.adoc_ file.
...
Attachment files are saved in the [.path]_assets/attachments/_ in the same module where the page that links to that file is located.
...
If your documentation component contains asset files over X size, we do not recommend that you store them in a regular GitLab / GitHub repository.
...
These files are inserted into a page with the AsciiDoc include directive.

[#pages-dir]
== Pages

AsciiDoc document files go in the [.path]_pages/_ folder.
These files are automatically enlisted by Antora and converted to standalone HTML pages.

Need example of page output from ROOT and page output from other module.

[#partials-dir]
=== Partial AsciiDoc files

Partial AsciiDoc files are stored in [.path]_pages/_partials/_ in the same module where the page that the partial page gets embedded in is located.
You don't need to set the path to the [.path]_partials/_ directory in the header of the primary AsciiDoc file.
