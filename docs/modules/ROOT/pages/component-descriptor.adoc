...
The version key value isn't related to a xref:component-versions.adoc[repository branch name] in any way.
...
If the component doesn't have an [.path]_index.adoc_ page in the ROOT module, you must specify a start page explicitly in the component descriptor.
The `start_page` key accepts the xref:page-id.adoc[page ID structure] and must point to an existing page in the same component (e.g., `ROOT:home.adoc`).

If a start page isn't specified, and the component does not have a ROOT module with an [.path]_index.adoc_ page, the reader will be directed to a 404 page when navigating to that component from the component drawer in the sidebar on any page.
...
The nav key accepts a list of xref:navigation:register-navigation-files.adoc[navigation files].
