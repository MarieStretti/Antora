...
If your playbook contains private repositories that use SSH, you must have an SSH agent running with the identity (i.e., SSH key) you've linked to your account on the git host.
Otherwise, Antora will fail when cloning the repository.
...
In addition to SSH, Antora can also authenticate with a private repository over HTTP.
To learn more about how to configure private repositories in your playbook, refer to xref:playbook:configure-content-sources.adoc#private-urls[using private repositories].
