...
If your playbook is xref:playbook:private-repository.adoc[configured to fetch private repositories via SSH], you must run an SSH agent with the identity (i.e., SSH key) you've linked to your account on the git host.
...
See xref:playbook:private-repository.adoc#http-auth[Connect to a private repository using HTTP authentication] to learn more.
