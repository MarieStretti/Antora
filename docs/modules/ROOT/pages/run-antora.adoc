...
If your playbook is xref:playbook:private-repository-auth.adoc[configured to fetch private repositories via SSH], you must run an SSH agent with the identity (i.e., SSH key) you've linked to your account on the git host.
...
Antora can also authenticate with a private repository over HTTP/HTTPS.
See xref:playbook:private-repository-auth.adoc#http-auth[Connect to a private repository using HTTP authentication] to learn more.
