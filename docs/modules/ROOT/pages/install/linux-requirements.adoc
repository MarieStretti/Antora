...
To install Antora, you need the base build tools for your Linux distribution and Node 8 (including npm, which is typically bundled with Node).
...
*If the command returns a version less than 8.0.0*, upgrade to the latest Node 8 version using your package manager.
Or, if your package manager doesn't provide Node and you have nvm installed, go directly to <<upgrade-node,upgrade Node with nvm>> for instructions.
