...
include::page$install/linux-requirements.adoc[tag=node-intro]
...
If *the command returns an active Node LTS version*, you're all set to xref:install/install-antora.adoc[install Antora].
...
IMPORTANT: When using nvm for Windows, you must enter an exact version of Node until {url-nvm-windows}/issues/214[nvm-windows#214^] is resolved.
--
...
IMPORTANT: When using nvm for Windows, you must enter the full version of Node (i.e., `nvm install {version-node-patch}`) until {url-nvm-windows}/issues/214[nvm-windows#214^] is resolved.
