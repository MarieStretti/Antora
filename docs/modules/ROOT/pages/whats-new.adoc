...
When Antora launched, nodegit was the only JavaScript git client that provided the features Antora required for interfacing with git repositories (clone, fetch, resolve refs, walk trees, read blobs).
Despite the fact that it's a comprehensive git client, nodegit has proven to be tremendously difficult for users to install.
...
In fact, isomorphic-git is now used to drive all of the git tests in the test suite, and the tests now use proper remote clone and fetch operations for testing remote repositories.
...
This change makes it easier to manage authentication.
Antora no longer requires running an SSH agent.
...
If none of those options work for you, the documentation also shows how to implement your own xref:playbook:private-repository-auth.adoc#custom-manager[credential manager plugin].
...
* Attribute references in attrlist of include directive are replaced.
...
* Proper warnings if an include tag is mismatched, unclosed, or not found.
...
But don't worry, the old names will still work.
They're just considered deprecated and will be removed in the next major version.
...
It was taking 11 seconds to install Antora 1.1.1 from cache.
