...
This improvement is due to Antora's dependencies no longer relying on native system libraries.
...
In fact, isomorphic-git now drives all of the git tests in the test suite, and the tests use proper remote clone and fetch operations for testing remote repositories.
