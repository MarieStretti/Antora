...
// Manage docs versions like a pro

Use branches to release docs versions just like software versions.
Antora can find the branches in each repository and assemble the files into a versioned documentation collection.
...
=== No syntax plugins, extensions, or shortcodes needed
...
== Category: Site UI and Theme

=== Decoupled UI = Happy Web Team

Since Antora can access multiple repositories, it makes sense that the site UI is stored in its own repository.
The web team can use their own workflow and development schedule without having their files mixed with documentation files.

=== UI Hot Swap

Switch between site UIs just by changing the UI address in the playbook.
Since site UIs can be stored in separate repositories and branches, developing, testing, and publishing new themes has no impact on your documentation files or their repositories.
...
A site's navigation is stored in AsciiDoc files right along with your documentation files.
Links are added to the navigation using the same cross referencing syntax used in the pages.
There's no special syntax or UI template logic to learn.

=== No content or organization restrictions

Add regular (non-linked) text, images, and links to pages not in your docs site to the navigation.
Organize the links in your site navigation in the order and nesting arrangement of your choice.
