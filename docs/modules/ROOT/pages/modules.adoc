...
When the documents stored in the [.path]_pages_ directory of ROOT are published to a documentation site, they're promoted a level above any other modules`' pages in that component's URL.

Let's say the component illustrated above is the documentation for your newest software product, _Hyper Lemur_.
...
The component name segment, in this case _hyperlemur_, is set using the `name` key in the xref:component-descriptor.adoc#name-key[component descriptor].
The page name segment is the basename of the AsciiDoc file.
Notice that the name of the module, `ROOT`, isn't in the URL.
Files that are stored directly in a ROOT module are published at the root of the component.
...
A partial is inserted into a page using the xref:asciidoc:include-partial.adoc[AsciiDoc include directive and the partial's resource ID].
