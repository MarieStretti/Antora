...
By default, Antora search a component for a file named `index.adoc`, first in the ROOT module, and then if it doesn't find it there, then in any other modules in that component.
Antora uses the index file as the root of a component.
If you don't want to name a file index.adoc in a component, then you must set the start_page key and explicitly assign a file to the key.
The value uses the page coordinate system.
You must specify what module the start page is located in and the path to the appropriate file.
...
IMPORTANT: The start_page key and value is required when a component doesn't contain an `index.adoc` file, otherwise Antora will issue an error message.
