...
:url-issues: {uri-repo}/issues
:url-chat-dev: https://gitter.im/antora/dev

Antora operates on macOS, Windows, and most Linux distributions.
It should also operate as designed on common VM environments and cloud platforms.

//There's also an Antora Docker container.
...
.Linux Distributions
====
While we only continuously test Antora on Alpine, Debian, and Fedora, it should operate as expected on most Linux distributions.
====
...
[caption="ANTORA STATS"]
NOTE: At last count, Antora had {test-count} tests.
And we add more every week.
We're just a teeny-tiny bit obsessed with writing quality tests and achieving complete test coverage.
...
Or try out the Antora Docker container:

* xref:antora-container.adoc[Antora Docker container]
