...
Produce sitemap::
Antora generates a site map that can be used as an internal report or published as a site page.

Publish site::
Antora writes the generated pages to a destination(s) specified in the playbook.
More than one destination may be specified, and different destination types are supported.
Destination types include local and remote filesystems, ZIP archives, and others.
