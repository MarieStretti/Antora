...
Publish site::
Antora writes the generated pages to a default or xref:playbook:output-keys.adoc#output-dir[user-specified location].
The site can be published to multiple locations and in multiple formats, such as a ZIP archive, using xref:playbook:output-keys.adoc#output-provider[built-in and custom destination providers].
