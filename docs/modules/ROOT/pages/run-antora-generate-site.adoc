...
Once you've xref:playbook:configure-playbook.adoc[configured your playbook], it's time to run Antora.
//Now that you've told Antora which content to use and which UI to apply to that content, it's time to run it.

== Run Antora

You run Antora by passing the `generate` command to Node.
