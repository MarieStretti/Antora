= Navigation List Item Content and Syntax
...
:keywords: nav.adoc, menu labels, navigation labels, menu entries
// Settings
:idprefix:
:idseparator: -

List titles and items in a navigation source file can contain several types of content, such as text, images, and icons.
In most cases this content will link to a page or resource.

== Linking methods

When content is linked, also known as anchored, it means that when the website visitor clicks on the content they are taken to another page or resource.
...
Cross references in a navigation source file use the same syntax and page ID construction as cross references in the page content.
Just set up the `xref` macro, insert the page ID of the destination page, and add the link content.
The link content can be text, an image, and/or an icon.
This is the content that is visible to the visitor in the component navigation menu.
This content is also displayed in the breadcrumbs at the top of the destination page.
When they click on the link content, they will be routed to the destination page specified by the page ID in the xref.

*Whether in the content of a page or in a navigation source file, when a page is part of your site, you should always create links to it using the Antora cross referencing system.*

=== In-module page links

The most common items in a navigation source file are cross references to pages in the module where the navigation file is stored.
...
=== Out-module and out-component page links

Using the xref:asciidoc:page-to-page-xref.adoc[appropriate page ID pattern], you can link to any document in the site from a navigation source file, whether it's stored in the same module or not.
...
A navigation source list can contain links to websites and resources that are not part of your site.
...
Whether linked or unlinked, the text you want displayed in your navigation menu can be formatted using inline AsciiDoc markup.
