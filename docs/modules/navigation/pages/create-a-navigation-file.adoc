...
* [x] How to add regular text and links to other sites to a navigation list.
...
Each top-level item (`{asterisk}`) can have up to five levels of nested navigation.
...
However, using the xref:asciidoc:page-to-page-xref.adoc[appropriate xref macro patterns], you can include references to any document in the site, whether it's stored in the same module or not.
