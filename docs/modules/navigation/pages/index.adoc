...
=== 1 Input: Navigation source file

The navigation for a component is described by at least one AsciiDoc file.
However, as seen in the diagram, you can have as many navigation source files as you need.
Creating a navigation source file per module is a common organizational structure we see.
A navigation source file contains one or more AsciiDoc lists.
Each item in the list can be the link to a documentation page, normal text, an image, icon, or the URL to a website page that isn't part of the documentation site.
Documentation page links are created using the same cross referencing syntax used in the pages themselves.

When you want the contents of a navigation source file to be published to a component's menu, you need to register the source file in the appropriate component descriptor file.

=== 2 Input: Nav key register

Navigation source files are registered in the component descriptor file under the nav key (`nav`).
List the files in the order you want them to be displayed in when they're published to the component menu on your site.

When you run Antora, it will only use the navigation source files you registered to create the published component menu.

=== 3 Publish: Component menu

Antora collects the contents of each registered navigation source file, puts the contents in the order the files are listed under the `nav` key, and publishes a fully assembled menu for the component to the site.
The behavior and style of the menu is dictated by the UI bundle specify in your playbook.

// screenshot of component menu

Antora carries out this navigation assembly process for each version of each component it locates in the repositories, branches, and tags listed under `sources` in the site's playbook.

=== 4 Publish: Component version selector

When you have several components and several versions of one or more components, Antora will assemble an aggregated list of the components and their versions.
...
In the default UI, this list is displayed in the drawer that is located at the bottom of any component menu.
When a reader clicks on the drawer, it expands upwards and the reader can then select the component and version of that component they want to see.

=== 5 Publish: Page version selector

If a page is available in more than one version of a component, a dropdown selector is visible on that page.
From this selector, the reader can navigate between other versions of that page.
...
* Navigation list item syntax: xrefs, text, text styles, images, URLs, and more
* Navigation list structures: nesting depth, single list, multi-list, titled versus non-titled lists
* Naming and organizing navigation files
* Navigation files and the AsciiDoc include directive
* The nav key and registering navigation files
* Component menu, home icon, and index / start page behavior
* Breadcrumbs
