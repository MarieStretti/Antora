...
A different number scheme is provided for each level of nesting.
...
When you're list doesn't need to be numbered, use an unordered list.
...
Let's look at an example of a list that contains a variety of content and list item types.
...
....
<1> A list continuation is a plus symbol (`{plus}`) symbol on a line by itself that is immediately adjacent to the block being attached.

To add additional paragraphs or blocks to a list item, you must "`attach`" them with a [.term]_list continuation_ (`{plus}`).
Using the list continuation, you can attach as many AsciiDoc elements to a list item as you require.
...
You can find xref:labeled-list.adoc#complex[another complex list example] on the description list page.
