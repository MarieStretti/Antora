...
To include a xref:page:index.adoc#partial[partial AsciiDoc file] into a standard page, use the include directive with a target starting with the attribute reference `+{partialsdir}+`, followed by the relative path of the partial file.
...
Partial AsciiDoc files should be saved in the xref:ROOT:modules.adoc#partials-dir[_partials_ directory] (or xref:ROOT:modules.adoc#partials-dir[_pages/_partials_ directory]) inside the current module.
...
Instead of using a partial, you can insert the contents of a xref:page:index.adoc#standard[standard page] into another standard page.
...
* {url-include}[Using the AsciiDoc include directive^]
