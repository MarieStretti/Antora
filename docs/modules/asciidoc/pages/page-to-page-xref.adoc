...
To link between documentation pages, you'll use the the AsciiDoc cross reference macro combined with a page ID.
...
NOTE: Antora's page ID system is one of its fundamental features.
It's integral to Antora's ability to publish a site to multiple environments and domains without you needing to modify your source content.
...
.Xref using a fully-qualified page ID
[source,asciidoc]
----
