...
A word or phrase is marked as italic when it is enclosed in a single set of underscores (`{underscore}word{underscore}`).
Constrained characters are marked as italic when they're enclosed in a double set of underscores (`char{underscore}{underscore}act{underscore}{underscore}ers`).
...
[source,asciidoc]
----
An italic _word_, and an _italic phrase_.

Italic c__hara__cter__s__ in a sentence.
----

Notice that you don't need to use double underscores when an entire word or phrase is marked as italic and directly followed by a common punctuation mark, such as `;`, `"`, and `!`.
...
How italic text appears on your site depends on the fonts loaded by your UI and any CSS styles it applies to the `<em>` HTML tag.
