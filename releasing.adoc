...
To publish a release, first make sure the <<Credentials,npm token>> is still valid and that the copyright year is current.
Then simply switch to the _releases_ branch, update the [.path]_releaserc_ file, and force push the branch to origin.
