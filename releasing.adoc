...
This is done by writing the token to an [.path]_.npmrc_ file inside the folder for each package (e.g., [.path]_packages/cli/.npmrc_).

After the package is published, commits (the tag and the corresponding commit to master) must be pushed from the CI job to the git repository.
...
The git client is then configured to push changes over SSH.
...
The values currently used are those for the release manager.

These secret variables are scoped to the releases environment.
...
Each release is triggered by pushing a commit to the releases branch, which in turn activates the release job in the CI pipeline.
The release job only runs on the releases branch.
