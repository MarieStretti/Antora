...
== 1.0.0-alpha.9 (2018-03-0x)

=== Added

* *redirect-producer:* First release of component responsible for generating HTTP redirects to a page from its registered aliases.
It can generate redirects using client-side and server-side producer such as `static` and `ngnix`.
* *content-classifier*: Compute URL for component version when index page in ROOT module is missing
* *content-classifier*: Automatically compute fallback URL for component version

=== Changed

* *content-classifier*: Move resolveStartPageUrl logic into ContentCatalog#addComponentVersion
* *content-classifier*: Add expandPageSrc helper in content catalog
