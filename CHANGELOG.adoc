...
* *content-aggregator*: Read credentials for private repository from git credential store file ($HOME/.git-credentials or $XDG_CONFIG_HOME/git/credentials) (#264)
* *content-aggregator*: Allow git credentials file path or contents to be configured using `git.credentials` key (path or contents) in playbook (#264)
* *page-composer*: Add next, previous, and parent properties to the page UI model to access adjacent pages (#233)
* *cli*: Add `--git-credentials-path` option or `GIT_CREDENTIALS_PATH` environment variable to override location of git credential store file (#264)
...
* *content-aggregator*: Drop support for SSH as a transport protocol / authentication mechanism (#264)
