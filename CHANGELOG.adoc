...
* *content-classifier*: Don't publish files with a leading underscore in filename or inside a directory with a leading underscore
* *content-classifier*: Don't drop _attributes.adoc files found inside pages directory from content catalog
* *document-converter*: Don't convert documents that don't have an out property
* *playbook-builder*: Remove automatic migration of legacy ui schema in playbook data
