...
* *content-aggregator*: Compute edit URLs for pages taken from repositories hosted at pagure.io (#283)
* *content-classifier*: Look for partial files in the partials folder inside the module in addition to pages/_partials (#254)
...
* *content-classifier*: Allow display version to be set in component descriptor; assign displayVersion property to component version (#288)
* *content-classifier*: If prerelease property on component version is a string and display version is not set, automatically construct and assign display version (#288)
...
* *navigation-builder*: Attach navigation (set) to each component version to make it accessible from template (#281)
* *page-composer*: Pass the origin information for a page to the UI model as `page.origin` (#293)
* *cli*: Add --generator option to specify a custom site generator library or script (#178)

=== Changed

* *content-aggregator*: Set component title to name if title is not defined
