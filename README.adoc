...
* Fedora: `dnf install libcurl-devel openssl-devel python`
...
NOTE: If you prefer Yarn over npm, you can choose to install Antora using `yarn global add @antora/cli@{version-antora}`.
...
[subs=attributes+]
 $ BUILD_ONLY=true npm install -g @antora/site-generator-default@{version-antora}
