...
IMPORTANT: The release versioning, scheduling, and support policies described in this section will take affect for the packages in this repository once Antora CLI v1.0.0 and Antora default site generator v1.0.0 are released.

The Antora CLI and Antora default site generator packages are released together and follow the https://semver.org[semantic versioning] rules.
Each Antora release is versioned *major.minor.maintenance*.
...
Major releases require architecture iterations that are available for community review and comment.
Major releases will include numerous pre-release versions (major.minor.maintenance-alpha.n | -beta.n | -rc.n).
Once a release candidate (rc) has been thoroughly tested and no new bugs discovered, the stable release will be published.

=== Support

Each major version of the default Antora site generator is maintained for at least X years after the initial public stable release.
