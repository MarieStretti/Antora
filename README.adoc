...
To install Antora, you'll need Node 8 or 10 (including npm, which is bundled with Node) on your system.
You may also find the base build tools for your OS helpful (which includes `git`), though they're not required.

==== Node

Antora requires Node 8 or 10, the current long-term support (LTS) releases of Node.
While you can try newer versions, Antora is not currently tested on them.
...
If the command doesn't report a Node 8 or 10 version (e.g., v{version-node}), you don't have a suitable version of Node installed.

If Node 8 or 10 is provided by the package manager for your operating system, you are free to install Node that way.
However, we primarily recommend using {uri-nvm}[nvm] (Node Version Manager) to manage your Node installations.
