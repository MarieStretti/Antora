...
// Project documentation URIs, please keep alphebetized:
...
// External URIs, please keep alphebetized:
...
. *Join the project.*
Post to the {uri-gitter-antora-dev}[gitter antora/dev channel] requesting to be added to the GitLab project {uri-members}[Members] mentioning your GitLab id.
...
. In the issue tracker, label the issue as *[Progress] Preparing Change* and make sure you're assigned to it so that the rest of the team knows you're working on it.  If the issue is already assigned but not labeled as being worked on feel free to reassign it to yourself.
...
. *Make your changes.*
 ...
 .. If not already present, add yourself to the list of contributors in the package.json file for the package you're modifying.
