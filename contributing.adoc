...
:uri-arch-aggregator: {uri-project}/blob/master/packages/content-aggregator/devdocs/architecture-guidebook.adoc
:uri-arch-classifier: {uri-project}/blob/master/packages/content-classifier/devdocs/architecture-guidebook.adoc
:uri-arch-converter: {uri-project}/blob/master/packages/document-converter/devdocs/architecture-guidebook.adoc
:uri-arch-page: {uri-project}/blob/master/packages/page-generator/devdocs/architecture-guidebook.adoc
:uri-arch-playbook: {uri-project}/blob/master/packages/playbook-builder/devdocs/architecture-guidebook.adoc
:uri-arch-ui: {uri-project}/blob/master/packages/ui-loader/devdocs/architecture-guidebook.adoc
// External URIs:
...
This project is hosted on GitLab in the {uri-org}[Antora organization].
This is the official home of the project and all development, issue management, and pipeline integration is done here.
...
If you have an idea for an improvement or have discovered an error or omission, jump onto the issue tracker.
Review the issue list and see if someone has already filed an issue about that subject.
If an issue already exists, feel free to join the conversation.
If an issue doesn't exist, please don't hesitate to create one.

If you want to help complete an issue by contributing code, documentation, or other enhancements, review our contribution workflow and requirements and then submit a merge request for review.
...
*Antora is an issue-driven project.*
*A merge request (MR) should always address an open issue.*
...
.. Applicable <<arch-guide,architecture guidebooks>>
.. Applicable <<adr,ADRs>>
...
. Prior to submitting a MR, *rebase your branch against the latest master* if needed, then push the branch to your fork.
...
When the MR is ready for final review, remove `WIP:` from the MR title.
...
If it fails, update your MR once you've corrected the problem(s).
...
. If you need to incorporate changes into your MR after you've pushed it, you can *append a new commit or rewrite an existing commit*, depending on what you think is most appropriate.

. When your MR is approved, a project member will merge it using a fast-forward merge (with merge commit).
.. The maintainer may decide to squash review commits.
Only the original work is guaranteed to be preserved.
...
[#code-workflow]
== Code Contribution Workflow and Requirements

. Set up your <<set-up-workspace,development workspace>>.
. Review any applicable <<arch-guide,architecture guidebooks>>.
...
. Run the <<run-tests,test suite>> (which includes eslint) and correct any errors.
. Commit your changes.
. <<run-prettier,Run prettier>>.
.. Prettier's format task will modify your files, so be sure to commit your changes before running it so you can review and rollback if necessary.
. Commit any formatting changes completed by prettier.
. Push to your fork and open a merge request.
////
. If your MR doesn't pass the CI pipeline, update your MR once you've corrected the problem(s).
. A project member will review your MR and request changes if needed.
Checkout the <<mr-approval,MR approval guidelines>> if you want to see how your MR will be evaluated.
.. When incorporating the changes into your MR, you can append a new commit or rewrite an existing commit, depending on what you think is most appropriate,
. When your MR is approved, a project member will merge it using a fast-forward merge (with merge commit).
.. The maintainer may decide to squash review commits.
Only the original work is guaranteed to be preserved.
////
...
A new ADR should be proposed prior to adding, removing, upgrading or significantly changing pipeline dependencies, frameworks, tools, environments, or CI, CD, and release processes.
...
* its inputs, outputs and high-level functionality
...
.Current Architecture Guidebooks
[%hardbreaks]
{uri-arch-playbook}[Playbook Builder]
{uri-arch-aggregator}[Content Aggregator]
{uri-arch-classifier}[Content Classifier]
{uri-arch-converter}[Document Converter]
{uri-arch-ui}[UI Loader]
{uri-arch-page}[Page Generator]
...
The style rules are enforced with eslint (not StandardJS), and the code is formatted with prettifier.
...
* Use SCREAMING_SNAKE_CASE for constant names (not a `const` declaration, but rather a true constant).
// * we might consider defining all constants for a package in lib/constants.js
* Keep all `const` declarations together at the top of the file (no blank lines), and sort them alphabetically, unless there is a reason they can't be.
* Functions should be defined as `const` with a function shorthand `const fn = () => {}`.
// * or maybe we want to enforce `fn () { }` (see https://eslint.org/docs/rules/func-style)
// * define whether functions should be defined at top or bottom of file

When you <<run-tests,run the test suite>> it will automatically lint (i.e., check) your JavaScript code first.
...
Run prettier *after* you commit all your code changes because the format task will modify your files.
...
You can run the whole <<run-tests,test suite>> manually or continuously, and select specific tests to run.
...
// NOTE nyc is currently disabled
...
This filter works for all gulp tasks, including `lint` and `test-watch`, which are covered later.
